...
import { itemApi, savedApi, cartApi } from './services/api';
...
const ItemDetailsPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const checkIfSaved = async () => {
            ...
            try {
                const response = await savedApi.check(parseInt(itemId));
    ...
    const removeWatcher = async () => {
        if (!itemId) return;
        try {
            await itemApi.decrementWatchers(parseInt(itemId));
        } catch (error) {
    ...
    const fetchSimilarItems = async (category: string) => {
        ...
        try {
            const response = await itemApi.getByCategory(category);
    ...
    const handleSaveListing = async () => {
        ...
        try {
            const response = await savedApi.save(item.id);
    ...
    const handleAddToCart = async () => {
        ...
        try {
            const response = await cartApi.add(item.id, quantity);
